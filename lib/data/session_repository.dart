...
class SessionRepository {
  ...
  Future<void> flushProgress(int sessionId, int durationSeconds) async {
    ...
    await (_db.update(_db.playSessions)..where((s) => s.id.equals(sessionId)))
  ...
  Future<void> commit(
    ...
    await (_db.update(_db.games)..where((g) => g.id.equals(gameId))).write(
      GamesCompanion.custom(
        totalPlaySeconds: _db.games.totalPlaySeconds + Variable(totalSeconds),
        lastPlayedAt: Variable(endedAt),
      ),
    );
  }
